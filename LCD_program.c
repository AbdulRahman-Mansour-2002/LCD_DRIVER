...
void PLCD_voidEnable(LCD_t* LCD){
	MGPIO_voidSetPinValue(LCD->ControlPorts[2],LCD->ControlPins[2],GPIO_HIGH);
	MCLOCK_voidDelayMS(2);

	MGPIO_voidSetPinValue(LCD->ControlPorts[2],LCD->ControlPins[2],GPIO_LOW);
	MCLOCK_voidDelayMS(2);
...
void PLCD_voidSetReadMode(LCD_t* LCD){
	...
	for(u8 i = 0; i < Local_u8Pins ; i++)
		MGPIO_voidSetPinDirection(LCD->DataPorts[i],LCD->DataPins[i],GPIO_INPUT);
}
...
void PLCD_voidSetWriteMode(LCD_t* LCD){
	...
	u8 Local_u8Pins = LCD->Mode == LCD_MODE_4_BIT?	4:8;
	for(u8 i = 0; i < Local_u8Pins ; i++)
	MGPIO_voidSetPinDirection(LCD->DataPorts[i],LCD->DataPins[i],GPIO_OUTPUT);
...
void HLCD_voidInit(LCD_t* LCD){
	...
	//pins are output
	for(u8 i = 0; i< 3; i++){
		MGPIO_voidSetPinDirection(LCD->ControlPorts[i],LCD->ControlPins[i],GPIO_OUTPUT);
	}
	for(u8 i = 0; i< Local_u8Pins; i++){
		MGPIO_voidSetPinDirection(LCD->DataPorts[i],LCD->DataPins[i],GPIO_OUTPUT);
	}
